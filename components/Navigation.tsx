...
export const Navigation: React.FC<NavigationProps> = ({ onNavigate, currentView, userInfo }) => {
  ...
  if (['landing', 'login', 'signup', 'create-hub', 'join-hub', 'forgot-password', 'hub-selection', 'waiting-approval', 'email-verification'].includes(currentView)) return null;
  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-[#F4F1EC] shadow-sm py-4 px-6 md:px-12 flex items-center justify-between border-b border-[#EFEAE3]">
      <div className="flex items-center gap-3">
        {/* Logo */}
        <div className="flex items-center gap-2 group transition-transform">
          <LogoIcon className="w-10 h-10" />
          <span className="text-xl font-bold text-[#2E2A4A] tracking-tight">MedTrack</span>
        </div>
      </div>
